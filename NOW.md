...
- [ ] **Phase 1: Structured Draft Entity**: Refactor `GeneratedArticle` to use an array of `DraftBlock` objects instead of a single string. Update `agentColumnist` to return JSON blocks.
- [ ] **Phase 2: KI-Linter & Micro-Actions**: Create `agentEditor` to analyze blocks. Build `TheEditorPanel` UI in The Bullpen. Implement block-level rewrite/expand actions.
