...
**Übersicht:** 62 Tasks · 6 TODO · 0 IN-PROGRESS · **1 BLOCKED** · 0 REVIEW · 38 DONE · **17 PARKED**
> **Foundation Track Tier 1 KOMPLETT** (alle ohne-Mensch baubaren Substrate). Verbleibende 6 TODO sind **alle gated**: `T-1.0.4`/`T-3.3.0` (brauchen deine Convex-/OAuth-Zugänge) · `T-4.1.1`/`T-4.3.1`/`T-4.2.1` (bauen auf `users` aus `T-3.3.0`) · `T-4.2.2` (gated `T-4.0.1` Plattform, BLOCKED).
...
| T-2.1.4 | Dormante Agenten verdrahten | DONE | — | A4 — Kein toter Export im Hauptpfad mehr: `agentPersonaSpeak` → Debatte verdrahtet (T-2.3.1); `agentExtractSeedClaims`/`agentSearchIndependentSources` laufen im manuellen Seed-Flow (`useNewsroomState`); `agentLayoutDesigner` läuft im manuellen Publish (`usePublicationFlow`) — Einbindung in den **Autonomie-Loop** ist UI-gekoppelt & bewusst **geparkt** (Redesign-Welle). |
...
| T-2.3.1 | Mehr-Runden-Debatte | DONE | T-1.1.1 | Q4 A — `conductDebate` ist jetzt eine **echte Schleife** (`DEBATE_ROUNDS=2`): jede Persona spricht pro Runde via `agentPersonaSpeak` und reagiert aufs laufende Transcript → echte Friktion statt 1 fabrizierter JSON-Call; `agentDebate` synthetisiert nur noch Angles aus dem **echten** Transcript. Persistiert in neuer `debate_transcripts`-Tabelle (`saveDebateTranscript`, am Cron-Call-Site) + `getDebateTranscript`-Query. tsc/codegen/build grün. |
| T-2.3.2 | Personas differenziert | DONE | T-2.3.1 | `DEBATE_PERSONAS` (Critic/Technologist/Accelerationist, scharf distinkte Lenses, ids↔Angle-ids) zentral in `agentDebate.ts`; `agentPersonaSpeak` bekommt `lens`-Param → jede Stimme bleibt in-character + erzeugt Reibung (alte Persona-Inkonsistenz behoben). |
| T-2.4.1 | Volle Provenienz-Kette | DONE | T-1.3.1 | Q11 — `drafts.provenanceChain` (storyId, `sourceSignalIds` aus `intentTrace`, `debateTranscriptId`, `chosenLens`, capturedAt) am autonomen Draft erfasst → Glass-Box von Snapshot auf **volle Kette Signale→Debatte→Entscheidung** erweiterbar. `saveDraft` nimmt die Kette (optional, additiv). Claims-für-autonome-Drafts = dokumentierte Folge-Erweiterung (Seed-Claims laufen bereits via T-1.3.1). tsc/codegen/build grün. |
...
| T-2.6.1 | Signal-Cache | DONE | T-1.1.2 | S2/Zero-Token — Kern-Verschwender war: `autonomousActions` embeddet **jedes** gefetchte Item **vor** der Dedup → RSS liefert dieselben URLs pro Sweep neu = wiederholte Embedding-Calls. Fix: `findSignalByUrl`-Vorabcheck **vor** `generateEmbedding` → bekannte URLs überspringen den teuren Call (`addSignal` dedupt nur den Write). Log zählt `skippedKnown`. (`generateEmbedding` ist nur Transport-Delegate, kein Modul-Cache — Analyse war hier ungenau.) Threshold-Zentralisierung (A2/A3) = kleiner Folge-Cleanup. tsc/codegen/build grün. |
...
| T-3.2.1 | Draft-Versionierung | DONE | — | Q3 B — `draft_revisions`-Tabelle (immutable v1→vN) + `saveDraft` schreibt Genesis-Revision (#1) + neue `reviseDraft`-Primitive (patcht Draft in-place & hängt Revision an) + `getDraftRevisions`-Query. Additiv, bricht den `saveDraft`-Churn nicht; v1→v2-**Anzeige** (Critics' Corner, T-3.2.2) = PARKED. tsc/codegen/build grün. |
...
| T-3.4.0 | Zeitreihen-Design | DONE | — | Lücke G1 — `story_snapshots`-Tabelle (storyId/snapshotAt/title/summary/keyEntities/memberCount/altitudeTags/centroidHash) + `captureStorySnapshot`-Mutation (zählt Member via `by_storyId`) + `getStorySnapshots`-Query; Initial-Snapshot beim Cluster-Bau. Drift-Analyse-Service + Viz → PARKED (T-3.4.2, Redesign-Welle). tsc/codegen/build grün. |
...
| T-4.4.1 | Digest-Generator | DONE | — | Q8 A — `lead_digests`-Tabelle + `generateLeadDigest`-Action (`digestActions.ts`, self-contained Gemini wie `synthesizeWithGemini`) synthetisiert die stärksten aufsteigenden Signale (`getTopSignalsByInnovation`, Innovation-Score, 7-Tage-Fenster) → `saveLeadDigest`; `getLatestDigest`-Query; **Wochen-Cron** (Mo 07:00). Render/Zustellung = T-4.4.2 (PARKED). tsc/codegen/build grün. |
...
- 2026-06-04 — **PIVOT „Substrat jetzt, Newsroom-Bühne später".** Newsroom-Redesign wird tiefgreifend → Oberfläche
  ...
  `/root/.claude/plans/ich-m-chte-erst-mal-sunny-nest.md`.
- 2026-06-04 — **Foundation-Substrat-Push (Tier 1 komplett):** `T-3.5.1` Altitude · `T-3.4.0` Story-Snapshots ·
  `T-2.6.1` Zero-Token-URL-Check · `T-2.3.1`/`T-2.3.2` echte Mehr-Runden-Debatte + `debate_transcripts` ·
  `T-2.4.1` volle Provenienz-Kette am Draft · `T-2.1.4` dormante Agenten verdrahtet · `T-4.4.1` Digest-Generator ·
  `T-3.5.2` Meta-Ausgaben-Generator · `T-3.2.1` Draft-Versionierung. Alle tsc/codegen/build grün, auf `dev`.
  Rest gated auf OAuth-Secrets + Prod-Key (Mensch). `ArticleProvenance`-Snapshot (sources+claims)
