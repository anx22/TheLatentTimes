...
// --- DEBUG SCAFFOLD ---
export const DebugScaffold: React.FC<{ label: string; error?: string }> = ({ label, error }) => (
  <div className="w-full py-24 border-y border-dashed border-neutral-300 bg-neutral-50 flex flex-col items-center justify-center text-center select-none opacity-50 hover:opacity-100 transition-opacity">
    <span className="font-mono text-xs font-bold text-neutral-400 tracking-[0.2em] uppercase">
      #### {label} ####
    </span>
    {error && (
      <span className="mt-2 font-mono text-[10px] text-red-500 uppercase tracking-widest">
        ERR: {error}
      </span>
    )}
  </div>
...
export const Coverlines: React.FC<{ items: CoverlineItem[] }> = ({ items }) => (
  <div className="flex flex-col gap-2">
    {items.map((item, idx) => (
      <div 
        key={idx} 
        className="group cursor-pointer py-8 border-b border-black/10 first:pt-0 last:border-0 hover:pl-6 transition-all duration-500 ease-out animate-fade-in"
