...
export const LayoutEngine: React.FC<LayoutEngineProps> = ({ 
  ...
  const handleCanvasDragOver = (e: React.DragEvent) => { e.preventDefault(); if (e.dataTransfer.types.includes('application/x-modus-slot-id')) setIsDragOverCanvas(true); };
  const handleCanvasDrop = (e: React.DragEvent) => { e.preventDefault(); setIsDragOverCanvas(false); const blockId = e.dataTransfer.getData('application/x-modus-slot-id'); if (blockId && onBlockDelete) onBlockDelete(blockId); };
  const handleDragOver = (e: React.DragEvent, blockId: string) => { e.preventDefault(); e.stopPropagation(); if (dragOverBlock !== blockId) { setDragOverBlock(blockId); setActiveZone(null); if (e.dataTransfer.types.includes('application/x-modus-block')) setDragType('BLOCK_TYPE'); else if (e.dataTransfer.types.includes('application/x-modus-slot-id')) setDragType('BLOCK_MOVE'); else setDragType('CONTENT'); } };
  const handleDrop = (e: React.DragEvent, blockId: string) => { e.preventDefault(); e.stopPropagation(); setDragOverBlock(null); setDragType(null); const sourceBlockId = e.dataTransfer.getData('application/x-modus-slot-id'); if (sourceBlockId && onBlockSwap && sourceBlockId !== blockId) { onBlockSwap(sourceBlockId, blockId); return; } const blockType = e.dataTransfer.getData('application/x-modus-block'); if (blockType && onBlockTypeDrop) { onBlockTypeDrop(blockId, blockType); return; } const itemId = e.dataTransfer.getData('text/plain'); if (itemId && onContentDrop) { onContentDrop(blockId, itemId); } };
  const handleBlockDragStart = (e: React.DragEvent, blockId: string) => { if (!onBlockClick) return; e.dataTransfer.setData('application/x-modus-slot-id', blockId); e.dataTransfer.effectAllowed = 'move'; };
