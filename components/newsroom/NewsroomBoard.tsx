...
interface NewsroomBoardProps {
    ...
    onSelectStory: (story: any) => void; // Loosened to any to prevent strict inference errors
    onShipBatch: () => void;
    activeItemId?: string;
}
    ...
    return (
        <div className="w-full h-full bg-zinc-50/50 flex p-8 gap-6 overflow-x-auto">
            ...
            <div className="flex-1 flex flex-col min-w-[340px] max-w-[420px]">
                <ColumnHeader title="Production" count={safeWorking.length} color="zinc-900" />
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 pb-20">
                        {safeWorking.length === 0 && (
                    ...
                    {safeWorking.map((story: StoryArtifact) => (
                        <AssetCard 
                            key={story.id} 
                            type="STORY" 
                            data={story} 
            ...
            <div className="flex-1 flex flex-col min-w-[340px] max-w-[420px]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3 pb-20">
                    ...
                    {safeBasket.map((story: StoryArtifact) => (
                        <AssetCard 
                            key={story.id} 
                            type="STORY" 
                            data={story} 
                ...
                {safeBasket.length > 0 && (
                    <div className="absolute bottom-8 right-8 z-20">
                        ...
                        >
                            <span>Publish Batch ({safeBasket.length})</span>
                            <span>→</span>
                        </button>
                    </div>
                )}
