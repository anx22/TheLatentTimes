...
const DiversityBadge: React.FC<{ label: string; type: 'TOPIC' | 'FORMAT' | 'MEDIA' }> = ({ label, type }) => {
    let style = "bg-gray-100 text-gray-500";
    if (type === 'TOPIC') style = "bg-accent/10 text-accent border-accent/20";
    if (type === 'FORMAT') style = "bg-blue-50 text-blue-600 border-blue-100";
    if (type === 'MEDIA' && label !== 'TEXT') style = "bg-black text-white border-black animate-pulse";
    
    return (
        <span className={`px-2 py-0.5 text-[9px] uppercase tracking-widest font-bold border rounded-sm mr-2 ${style}`}>
            {label}
        </span>
    );
};
              ...
              <div className="relative z-10 max-w-[1536px] mx-auto px-6 md:px-16 pt-32 md:pt-64 pb-20 grid grid-cols-12 gap-8">
                  <div className="col-span-12 md:col-span-8">
                      <div className="flex gap-2 mb-8 items-center">
                          <span className="text-xs font-sans font-bold tracking-[0.25em] text-accent uppercase pl-1 border-l-4 border-accent mr-4">{data.category}</span>
                          {data.topic && <DiversityBadge label={data.topic} type="TOPIC" />}
                          {data.format && <DiversityBadge label={data.format} type="FORMAT" />}
                          {data.media_type && data.media_type !== 'TEXT' && <DiversityBadge label={data.media_type} type="MEDIA" />}
                      </div>
    ...
    <section className="bg-white text-black py-32 border-t border-black">
      <div className="max-w-[1536px] mx-auto px-6 md:px-16">
        ...
        <div className={`max-w-5xl mb-24 ${isCenter ? 'mx-auto text-center' : ''}`}>
          <div className={`flex flex-wrap gap-2 mb-8 ${isCenter ? 'justify-center' : 'items-center'}`}>
              <span className={`text-xs font-sans font-bold tracking-[0.25em] text-accent block uppercase ${isCenter ? '' : 'pl-1 border-l-4 border-accent mr-4'}`}>
